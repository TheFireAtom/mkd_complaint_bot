...
EXCEL_FILE_READ = "complaints_for_read.xlsx"

# Инициализация Excel
if not os.path.exists(EXCEL_FILE_WRITE):
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.append(["Дата", "ФИО", "Этаж", "Квартира", "Дом", "Домофон", "Проблема"])
    wb.save(EXCEL_FILE_WRITE)

# Функция для копирования данных из файла для записи в файл для чтения
def copy_file_for_read():
    shutil.copy(EXCEL_FILE_WRITE, EXCEL_FILE_READ)
...
async def get_user_details(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if 'fio' not in context.user_data:
        ...
        try:
            wb = openpyxl.load_workbook(EXCEL_FILE_WRITE)
            ws = wb.active
            ws.append([
            ...
            # Копируем обновленный файл для чтения
            copy_file_for_read()
